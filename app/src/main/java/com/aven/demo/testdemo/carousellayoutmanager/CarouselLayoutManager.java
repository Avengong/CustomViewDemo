...
public class CarouselLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    protected int scrollBy(final int diff, @NonNull final RecyclerView.Recycler recycler, @NonNull final RecyclerView.State state) {
        ...
        if (0 != resultScroll) {
            mLayoutHelper.mScrollOffset += resultScroll;
            Log.d("中心位置", " scrollBy  mCenterItemPosition: " + mCenterItemPosition  );
    ...
    public void onLayoutChildren(@NonNull final RecyclerView.Recycler recycler, @NonNull final RecyclerView.State state) {
        ...
        Log.d("中心位置", " onLayoutChildren mCenterItemPosition: " + mCenterItemPosition  );
        fillData(recycler, state);
    }
    ...
    private void fillData(@NonNull final RecyclerView.Recycler recycler, @NonNull final RecyclerView.State state) {
        ...
        recycler.clear();
        Log.d("中心位置", " fillData mCenterItemPosition: " + mCenterItemPosition + "  currentScrollPosition:" + currentScrollPosition );
...
//        final int centerItem = Math.round(absCurrentScrollPosition);
        final int centerItem = (int) Math.floor(absCurrentScrollPosition);

        if (mCenterItemPosition != centerItem) {
            Log.d("中心位置", " detectOnItemSelectionChanged mCenterItemPosition: " + mCenterItemPosition + "centerItem:" + centerItem );
...
//            dimenDiff = (getWidthNoPadding() - mDecoratedChildWidth) / 2;
            dimenDiff = (getWidthNoPadding() - mDecoratedChildWidth) * 4 / 10;
        }
        //noinspection NumericCastThatLosesPrecision
    ...
    protected double convertItemPositionDiffToSmoothPositionDiff(final float itemPositionDiff) {
        ...
        if (absIemPositionDiff > StrictMath.pow(1.0f / mLayoutHelper.mMaxVisibleItems, 1.0f)) {
